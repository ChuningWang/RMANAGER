...
class namelist_maker():
    ...
    def __init__(self,run_opts):
        ...
        self.restart_file = self.output_dir + '/' + self.confcase + '_rst_copy.nc'
    ...
    def read_in_namelist(self,namlist,parameter):
        ...
        parameter = ' ' + parameter + ' =='
        for line in lines:
            if (line.find(parameter) != -1) & (line[0] != '!'):
                if line.find('!') == -1:
                    out = line.split()[-1].replace('d','')
                else:
                    out = line.split('!')[0].split()[-1].replace('d','')
        return out
...
class script_maker():
    ...
    def create_script(self):
        ...
        for line in lines:
            ...
            line = line.replace('<NNODES>',str(self.nnodes))
...
class run_manager():
    ...
    def __call__(self,current_job,duration='1y',infiles=[],restart=False):
        ...
        #--- compute how many cores and nodes are needed ---#
        self.run_opts['ncores'] = self.compute_ncores()
        self.run_opts['nnodes'] = self.compute_nnodes()
    ...
    def compute_nnodes(self):
        nnodes = int(float(self.run_opts['ncores']) / float(self.run_opts['ptile']))
        return nnodes
...
class tidy_your_run():
    ...
    def __init__(self,current_job,run_opts,sub_opts):
        ...
        self.list_ftypes = ['avg','his','dia','sta']
        ...
        self.restart_file_copy = self.confcase + '_rst_copy.nc'
    ...
    def archive_rst(self):
        ...
        status = os_utils.execute('cp ' + self.output_dir + '/' + rstin + ' ' + self.output_dir + '/' + rstcp)
        # tidal filter files
...
class setup_simulation():
    ...
    def __init__(self,user=None):
        ...
        # define user
        self.login = os_utils.get_output('whoami')[0]
        if user is None:
            self.user = self.login
        else:
            self.user = user # to override if RMANAGER user is different from login (e.g. on yellowstone)
        ...
        if self.machine in ['workstation','triton16','triton24']:
            try:
                self.scratch = os_utils.get_envvar('SCRATCH')
            except:
                exit('SCRATCH environment variable misssing, please add to .bashrc')
        elif self.machine in ['yellowstone', 'cheyenne']:
            self.scratch = '/glade/scratch/' + self.login

        # find where RMANAGER is installed
        self.rmanager_root = None
        pythonpath = os_utils.get_envvar('PYTHONPATH')
        list_pythonpath = pythonpath.replace(':',' ').split()
        for dir in list_pythonpath:
            if (dir.find('RMANAGER') != -1 and dir.find(self.login) != -1):
                self.rmanager_root = dir.replace('src/python','')
                break # we want the first occurence of RMANAGER in PYTHONPATH
    ...
    def _run_naming(self):
        ...
        assist = ''
        while domain == '':
            domain = raw_input('What is the domain name (CCS1, NWA, ...) ? \n >>> ')
        self.roms_app = domain

        while (assist != 'y' and assist != 'n'):
            assist = raw_input('Do you want to use the naming assistant ? please answer y (yes) or n (no) \n >>> ')
        if assist == 'y':
            # read info for prompt
            while initials == '':
                initials = raw_input('What are your initials (e.g. John Doe -> JD) ? \n >>> ')
            while (cast != 'F' and cast != 'H'):
                cast = raw_input('Is this simulation an Hindcast (past) or Forecast (future) ? please answer H or F \n >>> ')
            while (has_ice != 'y' and has_ice != 'n'):
                has_ice = raw_input('Is this simulation running an ice model ? please answer y (yes) or n (no) \n >>> ')
            while (has_bgc != 'y' and has_bgc != 'n'):
                has_bgc = raw_input('Is this simulation running a BGC model ? please answer y (yes) or n (no) \n >>> ')
            while (has_fish != 'y' and has_fish != 'n'):
                has_fish = raw_input('Is this simulation running the fish model ? please answer y (yes) or n (no) \n >>> ')
            while type(run_number) != int:
                run_number = raw_input('What is the number of this experiment (number of times you run this domain) ? \n >>> ')
                try:
                    run_number = int(run_number)
                except:
                    print 'not a number'
            while (ref != 'R' and ref != 'S' and ref != 'T' ):
                ref = raw_input('Is this a Reference simulation, a Sensitivity experiment or a Test ? please answer R (reference), S (sensitivity) or T (testing) \n >>> ')
            # creating run name
            self.runname = domain.upper() + '-' + initials.upper() + '.' + cast + 'Co' 
            if has_ice == 'y':
                self.runname = self.runname + 'i'
            if has_bgc == 'y':
                self.runname = self.runname + 'b'
            if has_fish == 'y':
                self.runname = self.runname + 'f'
            self.runname = self.runname + str(run_number).zfill(2) + ref
            print 'Name suggested would be : ', self.runname
            while (you_ok != 'y' and you_ok != 'n'):
                you_ok = raw_input('Are you ok with this name ? please answer y (yes) or n (no) \n >>> ')

        if (you_ok == 'n') or (assist == 'n'):
            newname = ''
            while newname == '':
                newname = raw_input('Please define the name of the simulation yourself (and be cursed for it) \n >>> ')
